...
function AdminDashboard() {
  ...
  // Load both pending and approved properties
  const fetchData = async () => {
    ...
    try {
      const [pendingRes, approvedRes] = await Promise.all([
        ...
        fetch(`${import.meta.env.VITE_API_URL}/properties/admin/approved`, {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
      ...
      if (pendingRes.ok) {
        ...
        setPendingList(Array.isArray(pendingData) ? pendingData : []);
      }
  ...
  const handleDeleteApproved = async (id) => {
    if (!window.confirm("Are you sure you want to delete this approved property? This action cannot be undone.")) return;
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/properties/delete/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200">
            <div className="text-3xl font-bold text-green-600 mb-2">{approvedList.length}</div>
            <p className="text-gray-600">Live Listings</p>
          </div>
          <div className="bg-white p-6 rounded-2xl border border-gray-200">
            <div className="text-3xl font-bold text-purple-600 mb-2">{pendingList.length + approvedList.length}</div>
        ...
        <div className="mb-8 flex gap-4">
          ...
          >
            ✅ Live Listings ({approvedList.length})
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-2xl font-bold text-gray-900">Live Listings (Approved)</h2>
          </div>

          {/* Loading State */}
          {loading && (
            <div className="p-12 text-center">
              <p className="text-gray-600 text-lg">Loading approved properties...</p>
            </div>
          )}
          ...
          {!loading && approvedList.length === 0 && (
            <div className="p-12 text-center">
              <p className="text-4xl mb-4">🏠</p>
              <p className="text-gray-600 text-lg font-semibold">No approved listings yet!</p>
              <p className="text-gray-500">Properties will appear here after approval.</p>
            </div>
          )}
                ...
                <div key={prop._id} className="p-6 hover:bg-gray-50 transition flex justify-between items-start gap-6">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        <p className="text-sm text-gray-600">Posted by</p>
                        <p className="font-semibold text-gray-900">{prop.owner?.name || "Unknown"}</p>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3 flex-shrink-0">
                    <button
                      onClick={() => handleDeleteApproved(prop._id)}
                      className="px-6 py-3 bg-red-600 text-white font-bold rounded-lg hover:bg-red-700 transition transform active:scale-95"
                    >
                      🗑️ Delete
                    </button>
